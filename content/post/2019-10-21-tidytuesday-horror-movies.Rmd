...
Everything looks neat and tidy. We can see each feature that *Sorority Slaughterhouse* will get a 1 on once we convert this to a matrix. So let's go ahead and create our matrix.
...
Our model has a whopping `r comma(nrow(cv_lasso_model_tidy) - 1)` predictors! **(Intercept)** shows the base score, then we add whatever value is in the `estimate` field for each feature that a movie has. We end up with that movie's a predicted score. For example, if a movie is a **Drama** it will get 0.332 *added* to its predicted score, while a movie with **Veronica Ricci** in its cast will get 0.374 *subtracted* from its predicted score.

If we join the `cv_lasso_model_tidy` to our `by_feature` dataset, we can get predicted values for each movie. Let's see how *Sorority Slaugherhouse* does.
...
Based on what our model is telling us, director has the biggest impact on predicted score. Almost every top feature, both positive and negative, is who directed it. I guess there's something to be said for a director with (or without) a vision.
